...
async function ensurePackageLink(outputDir, packageName, packagePath) {
    ...
    try {
        await symlink(resolve(packagePath), linkPath, process.platform === "win32" ? "junction" : "dir");
    } catch (err) {
        if (err?.code !== "EEXIST") {
            throw err;
        }
...
async function main() {
    ...
    try {
        ...
        const { transpile } = await import(pathToFileURL(
            join(jcoWorkspace, "node_modules", "@bytecodealliance", "jco", "src", "api.js"),
        ));
        ...
        if (typeof runInterface !== "object" || runInterface === null || typeof runInterface.run !== "function") {
