use std::{env, process};
use wasi_tests::{
    assert_errno, create_file, create_tmp_dir, drop_rights, fd_get_rights, open_scratch_directory,
};
...
unsafe fn test_read_rights(dir_fd: wasi::Fd) {
    ...
    wasi::path_unlink_file(dir_fd, TEST_FILENAME).expect("removing readable_file");
}
...
fn main() {
    ...
    const DIR_NAME: &str = "path_open_read_without_rights_dir.cleanup";
    let dir_fd;
    unsafe {
